...
puntos <- function(){
  ...
  if(resultado ==""){c <- ""}
  if(c == "Tiebreak"){c <- as.integer(stri_sub(resultado,1,stri_locate_first_regex(resultado, "-")[1]-1)) + as.integer(stri_sub(resultado,stri_locate_first_regex(resultado, "-")[1]+1,stri_length(resultado)))}
  c
}
...
# Funcion que devuelve un dataframe de una linea para el URL correspondiente
Fila <- function(){
...
if(is.na(c6)){c6 <- Desconocido}else{
...
set1 <- min(13, as.integer(sets("gi_p1_set0")) + as.integer(sets("gi_p2_set0")))
set2 <- min(13, as.integer(sets("gi_p1_set1")) + as.integer(sets("gi_p2_set1")))
set3 <- min(13, as.integer(sets("gi_p1_set2")) + as.integer(sets("gi_p2_set2")))
set4 <- min(13, as.integer(sets("gi_p1_set3")) + as.integer(sets("gi_p2_set3")))
set5 <- min(13, as.integer(sets("gi_p1_set4")) + as.integer(sets("gi_p2_set4")))
...
n <- 1
for(n in 1:500000){
  if(dia == diai & mes == mesi & ano == anoi){break}
  n <- n+1
  if(mes < 10){mese <- stri_c(0, mes)}else{mese <- mes}
  if(dia < 10){diae <- stri_c(0, dia)}else{diae <- dia}
  urlm <- stri_c("https://www.tennisbetsite.com/results/", ano, "/", mese, "/", diae, ".html")
  texto <- try(as.character(xml_child(read_html(urlm), 2)))
  if (inherits(texto, "try-error")){}else{
  z <- 1
  listapartidos <- list()
  listaurl <- stri_locate_all_regex(texto, "scores/stats")
  if(is.na(listaurl[[1]][1])){}else{
  for(z in 1:length(listaurl[[1]])/2){
  listapartidos[z] <-   stri_sub(texto, listaurl[[1]][z]-30, listaurl[[1]][z]+34)
  z <- z+1}
  listapartidos <- unique(listapartidos)
  y <- 1
  for(y in 1:length(listapartidos)){
  url <- listapartidos[[y]]
  if(stri_locate_first_regex(url,"p")[1]==5){url <- stri_sub(url, 2, stri_length(url))}
  texto <- try(as.character(xml_child(read_html(url, verbose = T), 2)))
  mtray <- try(Fila())
  if (inherits(mtray, "try-error")){
    lista <<- c(lista,url)}else{
  fila <- Fila()
  if((fila[12] != 0 & fila[12] != "")& length(fila) == largofila){
    colnames(fila) <- c("FECHA",	"TIPO PARTIDO",	"TENISTA A",	"TENISTA B",	"TIPO TORNEO",	"TIPO SUPERFICIE",	"TORNEO",	"GENERO",	"MARCADOR GENERAL",	"CANTIDAD DE SETS", "GANADOR",	"MARCADOR SET 1",	"GAME 1",	"No PUNTOS",	"GAME 2",	"No PUNTOS",	"GAME 3",	"No PUNTOS",	"GAME 4",	"No PUNTOS",	"GAME 5",	"No PUNTOS",	"GAME 6",	"No PUNTOS",	"GAME 7",	"No PUNTOS",	"GAME 8",	"No PUNTOS",	"GAME 9",	"No PUNTOS",	"GAME 10",	"No PUNTOS",	"GAME 11",	"No PUNTOS",	"GAME 12",	"No PUNTOS",	"GAME 13",	"No PUNTOS",	"MARCADOR SET 2", "GAME 1",	"No PUNTOS",	"GAME 2",	"No PUNTOS",	"GAME 3",	"No PUNTOS",	"GAME 4",	"No PUNTOS",	"GAME 5",	"No PUNTOS",	"GAME 6",	"No PUNTOS",	"GAME 7",	"No PUNTOS",	"GAME 8",	"No PUNTOS",	"GAME 9",	"No PUNTOS",	"GAME 10",	"No PUNTOS",	"GAME 11",	"No PUNTOS",	"GAME 12",	"No PUNTOS",	"GAME 13",	"No PUNTOS", "MARCADOR SET 3", "GAME 1",	"No PUNTOS",	"GAME 2",	"No PUNTOS",	"GAME 3",	"No PUNTOS",	"GAME 4",	"No PUNTOS",	"GAME 5",	"No PUNTOS",	"GAME 6",	"No PUNTOS",	"GAME 7",	"No PUNTOS",	"GAME 8",	"No PUNTOS",	"GAME 9",	"No PUNTOS",	"GAME 10",	"No PUNTOS",	"GAME 11",	"No PUNTOS",	"GAME 12",	"No PUNTOS",	"GAME 13",	"No PUNTOS", "MARCADOR SET 4", "GAME 1",	"No PUNTOS",	"GAME 2",	"No PUNTOS",	"GAME 3",	"No PUNTOS",	"GAME 4",	"No PUNTOS",	"GAME 5",	"No PUNTOS",	"GAME 6",	"No PUNTOS",	"GAME 7",	"No PUNTOS",	"GAME 8",	"No PUNTOS",	"GAME 9",	"No PUNTOS",	"GAME 10",	"No PUNTOS",	"GAME 11",	"No PUNTOS",	"GAME 12",	"No PUNTOS",	"GAME 13",	"No PUNTOS", "MARCADOR SET 5", "GAME 1",	"No PUNTOS",	"GAME 2",	"No PUNTOS",	"GAME 3",	"No PUNTOS",	"GAME 4",	"No PUNTOS",	"GAME 5",	"No PUNTOS",	"GAME 6",	"No PUNTOS",	"GAME 7",	"No PUNTOS",	"GAME 8",	"No PUNTOS",	"GAME 9",	"No PUNTOS",	"GAME 10",	"No PUNTOS",	"GAME 11",	"No PUNTOS",	"GAME 12",	"No PUNTOS",	"GAME 13",	"No PUNTOS", "4",	"5",	"6",	"+7", "4",	"5",	"6",	"+7", "4",	"5",	"6",	"+7", "4",	"5",	"6",	"+7", "4",	"5",	"6",	"+7", "Breaks Jugador 1", "Breaks Jugador 2",	"URL")
    tabla <- rbind(fila, tabla)}}
  y <- y+1}
}}
if (dia > 1){
  dia <- dia -1
...
recuperacion <-  append(recuperacion, tabla)
